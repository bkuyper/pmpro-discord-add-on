...
success:function(g){1==g.status&&location.reload()},error:function(g){alert("Server error")}})});a.ajax({type:"POST",dataType:"JSON",url:etsPmproParams.admin_ajax,data:{action:"load_discord_roles"},beforeSend:function(){a(".spinner").addClass("is-active")},success:function(c){c.hasOwnProperty("code")&&50001==c.code&&"Missing Access"==c.message?a(".btn-connect-to-bot").show():"401: Unauthorized"==c.message||c.hasOwnProperty("code")?a("#connect-discord-bot").show().html("Error: Please check all details are correct").addClass("error-bk"):
